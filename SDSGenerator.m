...
% W3n = 3 * mu * J2 * RE^2 / 2 / a / eta^3 / E^3 / L * ((2-3*s^2) * (TA - l) + 3 * s^2 * (sin(2*(TA+g)) / 2));
...
% 
% W3n = substituteFunctions(W3n *  E^3 / factorial(3), rfun, TAfun, Eccfun);
% W6n = substituteFunctions(W6n * E^6 / factorial(6), rfun, TAfun, Eccfun);
% H0s = substituteFunctions(H0, rfun, TAfun, Eccfun);
% H3s = substituteFunctions(H3 * E^3 / factorial(3), rfun, TAfun, Eccfun);
% 
% % Find variable terms
% L3H3 = 0;
% for j = 1:4
%     L3H3i(j) = diff(H3s,x(j)) * diff(W3n,X(j)) - diff(H3s,X(j)) * diff(W3n,x(j));
%     L3H3 = L3H3 + L3H3i(j);
% end
% L3H0 = - diff(H0s,L) * diff(W3n,l);
% L3H0 = substitutePartials(L3H0, subPartialsInputs);
% L3H0 = substituteFunctions(L3H0, rfun, TAfun, Eccfun);
% L3L3H0 = 0;
% for j = 1:4
%     L3L3H0i(j) = diff(L3H0,x(j)) * diff(W3n,X(j)) - diff(L3H0,X(j)) * diff(W3n,x(j));
%     L3L3H0 = L3L3H0 + L3L3H0i(j);
% end
% L6H0 = - diff(H0s,L) * diff(W6n,l);
% 
% L3H3 = substitutePartials(L3H3, subPartialsInputs);
% L3L3H0 = substitutePartials(L3L3H0, subPartialsInputs);
% L6H0 = substitutePartials(L6H0, subPartialsInputs);
% H6 = factorial(6) / E^6 * (4 * L3H3 - L3L3H0 + L6H0);
% K06 = factorial(6) / E^6 * (4 * L3H3 - L3L3H0 + 2 * L6H0);
...
% K06 = factorial(6) / E^6 / 2 * J2^2 * -mu / a * RE^4 * 3 * eta / a^4 / eta^8 / 64 * (5 * (8 - 16*s^2 + 7 * s^4) + 4 * (2-3*s^2)^2 * eta + (5*s^4 + 8*s^2-8) * eta^2 + 2 * (15*s^2-14) * s^2 * e^2 * cos(2*g)); 
